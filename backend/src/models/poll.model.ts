...
/** Opción con el recuento del GROUP BY (pg devuelve los bigint como string). */
export type PollOptionRow = Row<'poll_options'> & { vote_count?: string | null };

// Builds the poll payload embedded into a message response.
// `myVotes` are the option ids the requesting user voted for.
export function toPollResponse(
  poll: Row<'polls'> | null | undefined,
...
export type PollResponse = NonNullable<ReturnType<typeof toPollResponse>>;
