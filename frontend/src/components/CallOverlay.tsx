...
interface Tile extends VideoTileProps {
  id: string;
}
...
function ActiveCall() {
  ...
  const prevSharerId = useRef<string | null>(null);
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-white/8 px-5 py-3">
        ...
        <Tooltip>
          ...
          <Tooltip.Content placement="bottom">
            <p>{isFullscreen ? t('call.exitFullscreen') : t('call.fullscreen')}</p>
          </Tooltip.Content>
        </Tooltip>
      </div>
...
function ControlButton({ active, onPress, on, off, label, highlightWhenActive = false }: ControlButtonProps) {
