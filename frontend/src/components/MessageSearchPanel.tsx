...
function Highlighted({ text, query }: { text?: string | null; query: string }) {
  const parts = useMemo<HighlightPart[]>(() => {
    if (!text) return [];
    if (!query) return [{ key: 0, match: false, value: text }];
...
export default function MessageSearchPanel({ conversationId, onClose, onJump }: MessageSearchPanelProps) {
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2">
        ...
        <div className="flex flex-col gap-1">
          {results.map((msg) => (
            ...
            >
              <div className="flex items-center gap-2">
                {/* La API de búsqueda de mensajes no manda una URL de avatar
                    resuelta (sólo sender_avatar_key) — igual que antes, este
                    avatar cae siempre a las iniciales. */}
                <UserAvatar
                  user={{ display_name: msg.sender_display_name }}
                  size="sm"
                />
