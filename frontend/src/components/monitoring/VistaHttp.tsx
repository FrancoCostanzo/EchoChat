...
import type { DashboardData, RouteStat, HttpError5xx } from '@/types/monitoring';

type IconType = ComponentType<{ className?: string }>;

const ACCENT_BG: Record<string, string> = { ...STATUS_BG_CLASSES, primary: 'bg-primary-50 dark:bg-primary-900/20' };
const ACCENT_TEXT: Record<string, string> = { ...STATUS_TEXT_CLASSES, primary: 'text-primary' };
...
const errorRateColor = (pct: number | null | undefined): string => {
...
function KpiCard({ icon, label, tooltip, value, unit, accent = 'primary', colorValue = false, footer }: KpiCardProps) {
  const IconEl = icon;
  return (
...
function LatencyBar({ label, caption, value, tooltip, max }: LatencyBarProps) {
...
const VistaHttp = ({ dashboard }: { dashboard?: DashboardData | null }) => {
  const http = dashboard?.http;
  const [routeLimit, setRouteLimit] = useState('10');

  const allRoutes: RouteStat[] = http?.routes || http?.topRoutes || [];

  const routeRows = useMemo(() => {
    const limited = routeLimit === 'all' ? allRoutes : allRoutes.slice(0, 10);
    return sortItems<RouteRow>(
      limited.map((r, id) => ({
        id,
        route: r.route,
        count: r.count ?? 0,
      ...
      (item, col) => item[col as keyof RouteRow],
    );
  }, [allRoutes, routeLimit]);

  const errorRows = useMemo<ErrorRow[]>(() => (
    (http?.recentErrors5xx || []).map((err: HttpError5xx, id: number) => ({
      id,
      route: err.route,
                    ...
                    {errorRows.map((item) => (
                      <Table.Row key={item.id} id={String(item.id)}>
                        ...
                        <Table.Cell className="text-center">
                          <Tooltip>
                            <Tooltip.Trigger>
                              <Chip size="sm" color="danger" variant="soft">{item.statusCode}</Chip>
                            </Tooltip.Trigger>
