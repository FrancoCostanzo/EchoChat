...
import { MONITOREO_TOOLTIPS as T } from './monitoreo.helpers';
import { LabelWithTooltip } from './monitoreo.ui';
 ...
 */
const gapHighlightPlugin: Plugin<'line'> = {
    id: 'gapHighlight',
    beforeDatasetsDraw(chart, _args, opts: GapHighlightOptions) {
...
const fmtDate = (d: Date): string => `${pad(d.getDate())}/${pad(d.getMonth() + 1)}`;
const fmtFull = (d: Date): string => `${fmtDate(d)} ${fmtTime(d)}`;
...
/** `MonitoringSnapshot` con los campos numéricos coercidos a `number` (llegan como string para las columnas numeric/bigint de Postgres) y timestamps ya parseados. */
type SnapshotRow = Omit<MonitoringSnapshot, MetricKey> & Record<MetricKey, number> & {
    ts: Date;
    /** Instante de arranque del proceso (ms) — null si la fila es vieja sin la columna. */
    procesoInicio: number | null;
};

const toRow = (row: MonitoringSnapshot): SnapshotRow => {
    const ts = new Date(row.fecha_registro);
    const numeric = {} as Record<MetricKey, number>;
...
const emptyMetrics = (): Record<MetricKey, null> =>
 ...
 */
const detectRestart = (prev: SnapshotRow, row: SnapshotRow): boolean => {
...
/** Punto de la serie: una fila real (con sus métricas) o un slot vacío que marca un corte. */
type SeriesPoint = (SnapshotRow | Record<MetricKey, null>) & {
    gap?: boolean;
    axisLabel: string;
 ...
 */
const buildSeries = (rawRows: MonitoringSnapshot[]) => {
    const rows = rawRows.map(toRow).sort((a, b) => a.ts.getTime() - b.ts.getTime());
    ...
    rows.forEach((row, i) => {
        if (i > 0) {
            const prev = rows[i - 1];
            const delta = row.ts.getTime() - prev.ts.getTime();
...
const buildChartJsData = (metric: Metric, series: SeriesPoint[]) => ({
...
// El objeto de opciones mezcla claves de chart.js con las que agregan
// chartjs-plugin-zoom y nuestro propio gapHighlightPlugin — ChartOptions<'line'>
// no las conoce a todas (ni vale la pena una module augmentation para esto),
// así que se castea en el punto de uso en vez de pelear cada string literal
// (mode: 'index', etc.) contra los unions de chart.js.
const buildChartOptions = (metric: Metric, fullLabels: (string | null)[], gapSpans: GapSpan[], restartMarkers: RestartMarker[]) => ({
    responsive: true,
...
interface MetricChartProps {
    metric: Metric;
    series: SeriesPoint[];
    fullLabels: (string | null)[];
    gapSpans: GapSpan[];
    restartMarkers: RestartMarker[];
}

const MetricChart = ({ metric, series, fullLabels, gapSpans, restartMarkers }: MetricChartProps) => {
    const chartRef = useRef<LineChartHandle | null>(null);
    const chartData = useMemo(() => buildChartJsData(metric, series), [metric, series]);
    const chartOptions = useMemo(
        () => buildChartOptions(metric, fullLabels, gapSpans, restartMarkers) as unknown as ChartOptions<'line'>,
            ...
            <Card.Header className="pb-0 flex items-center justify-between gap-2">
                ...
                <Tooltip>
                    ...
                    <Tooltip.Content placement="top">
                        <p>Restablecer zoom (o doble clic en el gráfico)</p>
                    </Tooltip.Content>
                </Tooltip>
            </Card.Header>
...
const hostOf = (row: MonitoringSnapshot): string => row.host || UNKNOWN_HOST;

interface VistaTendenciasProps {
    initialData?: MonitoringSnapshot[];
    isActive?: boolean;
}

const VistaTendencias = ({ initialData = [], isActive = true }: VistaTendenciasProps) => {
    const [range, setRange] = useState('24h');
    const [rows, setRows] = useState<MonitoringSnapshot[]>(initialData);
    ...
    useEffect(() => {
        ...
        const latest = rows.reduce<MonitoringSnapshot | null>(
    ...
    const loadHistory = useCallback(async (selectedRange: string) => {
        setLoading(true);
        setError(null);
        try {
        ...
        <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2 items-center">
                    ...
                    {hosts.map((h) => (
                        <Button
                            key={h}
                            size="sm"
                            variant={selectedHost === h ? 'primary' : 'ghost'}
            ...
            {loading && (
                <div className="flex flex-col items-center gap-2 py-12">
                    <Spinner />
                    <p className="text-sm text-default-500">Cargando historial...</p>
                </div>
            )}
