...
export const InfoTooltip = ({ content, placement = 'top' }: { content?: ReactNode; placement?: 'top' | 'bottom' | 'left' | 'right' }) => {
...
export const LabelWithTooltip = ({ label, tooltip, className = '' }: { label: ReactNode; tooltip?: ReactNode; className?: string }) => (
...
export const QuickStatCard = ({ label, icon, tooltip }: { label: ReactNode; icon?: IconType; tooltip?: ReactNode }) => {
  const IconEl = icon;
  return (
...
export const StatBlock = ({ label, value, unit, tooltip, className = '' }: StatBlockProps) => (
...
export const MetricItem = ({ label, value, unit, icon, progress = null, subtext = null, tooltip = null }: MetricItemProps) => {
  const IconEl = icon;
  return (
...
export const TableColumnHeader = ({ label, tooltip }: { label: ReactNode; tooltip?: ReactNode }) => (
...
export const HttpRouteLabel = ({ route }: { route?: string | null }) => {
    ...
    <span className="inline-flex items-center gap-2 min-w-0 max-w-full">
      {method && (
        <Chip
          ...
          className="h-5 min-w-0 px-1.5"
        >
          <Chip.Label className="font-mono text-[10px] font-bold px-0">{method}</Chip.Label>
        </Chip>
      )}
...
export const StatusCard = ({ title, status, icon, children, tooltip = null }: StatusCardProps) => {
  const IconEl = icon;
  ...
  return (
    <Card className="h-full">
      <Card.Header className="flex items-center justify-between gap-2">
        ...
        {status && (
          <Chip color={color} variant="soft" size="sm" className="capitalize shrink-0">
