...
import { MONITOREO_TOOLTIPS as T, sortItems, type SortDescriptor } from './monitoreo.helpers';
...
const VistaBaseDatos = ({ dashboard }: { dashboard?: DashboardData | null }) => {
  ...
  const waitStats = dbDetails?.waitStats;
  const cumulativeWaitStats = waitStats?.cumulativeSinceServerStart;

  const [sortCol, setSortCol] = useState('session_id');
  const [sortDir, setSortDir] = useState<SortDescriptor['direction']>('ascending');

  const connectionRows = useMemo(() => {
    const rows: ConnectionRow[] = (dbDetails?.connectionCount?.connection_details || []).map((conn, id) => ({
      id,
    ...
    return sortItems(rows, { column: sortCol, direction: sortDir }, (item, col) => item[col as keyof ConnectionRow]);
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
      <StatusCard title="Base de Datos" status={db?.status} icon={Database}>
        ...
        {cumulativeWaitStats && cumulativeWaitStats.length > 0 && (
          <>
            <hr className="border-divider my-4" />
            <p className="text-xs text-default-400 mb-2">{waitStats?.note}</p>
            <LabelWithTooltip
            ...
            <div className="space-y-3">
              {cumulativeWaitStats.slice(0, 5).map((stat, idx) => (
                  ...
                  <Table.Body>
                    {connectionRows.map((item) => (
                      <Table.Row key={item.id} id={String(item.id)}>
                        ...
                        <Table.Cell>
                          <Chip size="sm" color={item.status === 'active' ? 'success' : 'default'} variant="soft">
                            {item.status}
                          </Chip>
                        </Table.Cell>
