...
const RESULTADO_ORDEN = { success: 1, error: 2, pendiente: 3 };
const RESULTADO_ICON = { success: CheckCircle2, error: XCircle, pendiente: Clock };
const RESULTADO_LABEL = { success: 'success', error: 'error', pendiente: 'pendiente' };
...
const getCronSortValue = (item, column) => {
  switch (column) {
...
function SocketStat({ icon, label, value, tooltip, boxClass, iconClass, live = false }) {
  const IconEl = icon;
  return (
...
function WorkerStateRow({ icon, label, color, text }) {
  const IconEl = icon;
  return (
    <div className="flex items-center justify-between gap-2 py-1.5">
      ...
      <Chip color={color} variant="flat" size="sm">{text}</Chip>
    </div>
  );
}

const VistaCronSocket = ({ dashboard }) => {
  ...
  const cronRows = useMemo(() => {
    ...
    const rows = Object.entries(jobs).map(([key, job], id) => ({
      id,
      key,
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <Card.Header className="flex items-center justify-between">
            ...
            <Chip size="sm" variant="flat" color={(socket.activeSockets ?? 0) > 0 ? 'success' : 'default'}>
        ...
        <Card>
          ...
          <Card.Content className="p-4 space-y-1 divide-y divide-divider">
            <WorkerStateRow icon={Power} label="Modo" color={cronWorker.enabled ? 'primary' : 'default'} text={cronWorker.enabled ? 'Producción' : 'Todos los entornos'} />
      ...
      <Card>
        <Card.Header className="flex items-center justify-between">
          ...
          {cronRows.length > 0 && (
            <div className="flex items-center gap-2">
              <Chip size="sm" variant="flat" color="success">{successCount} ok</Chip>
              {errorCount > 0 && <Chip size="sm" variant="flat" color="danger">{errorCount} con error</Chip>}
            </div>
          )}
        </Card.Header>
        <hr className="border-divider" />
