...
const PRESENCE_RANK = { online: 0, away: 1, busy: 2, dnd: 3 };
const ROLE_I18N = {
...
function compareMembers(a, b) {
  const aActive = ACTIVE_PRESENCES.has(a.presence);
  const bActive = ACTIVE_PRESENCES.has(b.presence);
...
export default function PresenceAvatarStack({
  ...
  const { active, offline, onlineCount, visible, overflowCount } = useMemo(() => {
    ...
    const filtered = q
      ...
      : sorted;
    return {
      active: filtered.filter((m) => ACTIVE_PRESENCES.has(m.presence)),
      offline: filtered.filter((m) => !ACTIVE_PRESENCES.has(m.presence)),
  ...
  const isMe = (member) => member.user_id === currentUser?.id;
    ...
    return (
      ...
      >
        <div className="flex h-12 w-full shrink-0 items-center justify-center border-b border-white/8">
          <Tooltip content={t('chat.viewMembers')} placement="left">
            <Button
              isIconOnly
              size="sm"
              variant="ghost"
          ...
          {visible.map((member) => {
            ...
            return (
              <Tooltip
                ...
                content={`${name} · ${t(`settings.presenceOptions.${presence}`, presence)}`}
                placement="left"
              >
                <button
                  type="button"
