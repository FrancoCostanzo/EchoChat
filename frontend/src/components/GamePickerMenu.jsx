...
/* ─────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────── */
export default function GamePickerMenu({ conversationId, disabled, open, onOpenChange }) {
  ...
  useEffect(() => {
    ...
    const onDown = (e) => { if (rootRef.current && !rootRef.current.contains(e.target)) onOpenChange(false); };
  ...
  const startGame = useCallback(async (kind, extra = {}) => {
    if (busy) return;
    setBusy(true);
    try {
      ...
      toast.danger(err?.message || t('game.startError'));
    } finally {
      setBusy(false);
    }
  }, [busy, conversationId, onOpenChange, t]);

  const handlePick = useCallback((kind) => {
    if (kind === 'hangman') {
  ...
  return (
    <div className="relative" ref={rootRef}>
      <Tooltip content={t('game.play')} placement="top">
        <Button
          isIconOnly
          variant="ghost"
          isDisabled={disabled}
