import { useCallback, useEffect, useMemo, useRef, useState, type ComponentType, type ChangeEvent, type KeyboardEvent, type ReactNode } from 'react';
...
import { giphySearch, GIPHY_ENABLED, GIPHY_ATTRIBUTION, type GiphyResult } from '@/lib/giphy';
import { stickerApi } from '@/lib/endpoints';
import type { UserStickerResponse, StickerPackResponse, StickerCollection, UpdateStickerRequest } from '@/types/sticker';

/** Firma mínima de `t` que usan estos subcomponentes — evita pasar toda la instancia de i18next por props. */
type TFunc = (key: string) => string;

/** Lo que le llega a onPick al mandar un sticker propio (distinto del shape de un GiphyResult). */
export interface CustomStickerPick {
  source: 'custom';
  kind: 'sticker';
  object_id: string;
  url: string | null;
...
export type PickedItem = GiphyResult | CustomStickerPick;
...
type PickerKind = 'sticker' | 'gif';

const KINDS: { key: PickerKind; icon: ComponentType<{ size?: number }> }[] = [
...
const EMPTY_COLLECTION: StickerCollection = { packs: [], stickers: [], recents: [] };
const NO_PACK = 'none';

function isPortaledOverlay(target: EventTarget | null): boolean {
...
interface GifGridProps {
  ...
  error: boolean;
  needsKey: boolean;
  onPick: (item: GiphyResult) => void;
  t: TFunc;
}

/* ── GIF results grid (Giphy) ── */
function GifGrid({ items, loading, error, needsKey, onPick, t }: GifGridProps) {
  if (needsKey) {
    return (
...
interface StickerTileProps {
  ...
  managing: boolean;
  onPick: (sticker: UserStickerResponse) => void;
  onEdit: (sticker: UserStickerResponse) => void;
  t: TFunc;
}

/* ── A single sticker tile ── */
function StickerTile({ sticker, managing, onPick, onEdit, t }: StickerTileProps) {
  return (
    <div className="echo-checker relative aspect-square overflow-hidden rounded-lg">
      ...
      >
        <img src={sticker.url ?? undefined} alt={sticker.name || ''} loading="lazy" className="max-h-full max-w-full object-contain" />
      </button>
...
interface StickerEditorProps {
  ...
  onSave: (id: string, fields: UpdateStickerRequest) => Promise<void>;
  onDelete: (sticker: UserStickerResponse) => Promise<void>;
  onClose: () => void;
  t: TFunc;
}

/* ── Editor overlay for one sticker: rename, keywords, pack, favorite, delete ── */
function StickerEditor({ sticker, packs, onSave, onDelete, onClose, t }: StickerEditorProps) {
  const [name, setName] = useState(sticker.name || '');
  const [keywords, setKeywords] = useState<string[]>(sticker.keywords || []);
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-3">
        <div className="flex justify-center">
          <div className="echo-checker flex h-24 w-24 items-center justify-center overflow-hidden rounded-lg p-1.5">
            <img src={sticker.url ?? undefined} alt="" className="max-h-full max-w-full object-contain" />
          </div>
        </div>
...
interface StickerPanelProps {
  ...
  searching: boolean;
  activePack: string;
  onActivePack: (pack: string) => void;
  onPick: (sticker: UserStickerResponse) => void;
  onEdit: (sticker: UserStickerResponse) => void;
  onAdd: () => void;
  onCreatePack: (name: string) => Promise<void>;
  onRenamePack: (pack: StickerPackResponse) => Promise<void>;
  onDeletePack: (pack: StickerPackResponse) => Promise<void>;
  t: TFunc;
}
        ...
        <div className="mb-2">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">
            {recents.map((s) => (
              ...
              >
                <img src={s.url ?? undefined} alt="" loading="lazy" className="max-h-full max-w-full object-contain" />
              </button>
            ))}
          </div>
        </div>
...
function PackChip({ active, onClick, children }: PackChipProps) {
  return (
    <button
      type="button"
      onClick={onClick}
...
export default function StickerGifPicker({ onPick, disabled, open, onOpenChange }: StickerGifPickerProps) {
  const { t } = useTranslation();
  const setOpen = useCallback((next: boolean | ((prev: boolean) => boolean)) => {
    onOpenChange(typeof next === 'function' ? next(open) : next);
  }, [open, onOpenChange]);
  const [kind, setKind] = useState<PickerKind>('sticker');
  ...
  const [collection, setCollection] = useState<StickerCollection>(EMPTY_COLLECTION);
  ...
  const [editing, setEditing] = useState<UserStickerResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const onDown = (e: MouseEvent) => {
      if (!rootRef.current || rootRef.current.contains(e.target as Node) || isPortaledOverlay(e.target)) return;
      setOpen(false);
    };
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (!(err instanceof DOMException && err.name === 'AbortError')) { setGifError(true); setGifs([]); }
      } finally {
  ...
  const handlePickGif = useCallback((item: GiphyResult) => { onPick(item); setOpen(false); }, [onPick, setOpen]);

  // Send a custom sticker and record its usage (feeds the recents row).
  const handlePickSticker = useCallback((s: UserStickerResponse) => {
    stickerApi.use(s.id).catch(() => {});
    onPick({ source: 'custom', kind: 'sticker', object_id: s.object_id, url: s.url, width: s.width, height: s.height });
    setOpen(false);
  }, [onPick, setOpen]);

  const switchKind = useCallback((k: PickerKind) => {
    setKind(k);
    setManaging(false);
    setQuery('');
  ...
  const handleSaveEdit = useCallback(async (id: string, fields: UpdateStickerRequest) => {
    try {
  ...
  const handleDeleteSticker = useCallback(async (s: UserStickerResponse) => {
    setEditing(null);
    try {
  ...
  const handleRenamePack = useCallback(async (pack: StickerPackResponse) => {
  ...
  const handleDeletePack = useCallback(async (pack: StickerPackResponse) => {
    try {
