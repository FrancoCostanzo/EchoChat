...
export default function PollMessage({ message, currentUserId, variant = 'other' }: PollMessageProps) {
  const { t } = useTranslation();
  const applyPollUpdate = useChatStore((s) => s.applyPollUpdate);
  const poll = message.poll;
  const [pending, setPending] = useState<string[]>([]); // local selection for multiple choice
