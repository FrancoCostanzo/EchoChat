...
import type { WallpaperEntry } from '@/types/wallpaper';
...
type WallpaperTab = 'presets' | 'color' | 'image';
...
export default function WallpaperPicker({ isOpen, onClose, scope, scopeKey, label }: WallpaperPickerProps) {
  const { t } = useTranslation();
  const { wallpapers, setWallpaper, removeWallpaper } = useWallpaperStore();
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const pendingUploadRef = useRef<string | null>(null); // uploaded in this session but not saved yet

  const [tab, setTab] = useState<WallpaperTab>('presets');
  ...
  const discardOrphanUpload = useCallback((objectId: string | null) => {
  ...
  useEffect(() => {
    ...
    if (current?.wallpaper_type === 'preset') {
      setTab('presets');
      setSelectedPreset(current.wallpaper_value ?? null);
    } else if (current?.wallpaper_type === 'color') {
      setTab('color');
      setSelectedColor(current.wallpaper_value ?? SOLID_COLORS[0]);
...
export function WallpaperPreview({ wallpaper, className = '' }: { wallpaper?: WallpaperEntry | null; className?: string }) {
  if (!wallpaper) {
    return (
      <span
  ...
  if (wallpaper.wallpaper_type === 'color') {
    return (
      <span
        className={`block rounded-md ${className}`}
        style={{ backgroundColor: wallpaper.wallpaper_value ?? undefined }}
      />
    );
  }
