...
import type {
  GameResponse, TicTacToeGameResponse, RpsGameResponse, HangmanGameResponse,
  GameMoveRequest, RpsChoice,
} from '@/types/game';
import type { MessageResponse } from '@/types/message';

type GameVariant = 'own' | 'other';

const LETTERS = 'abcdefghijklmnopqrstuvwxyz'.split('');
const RPS_CHOICES: { id: RpsChoice; emoji: string }[] = [
  { id: 'rock', emoji: '✊' },
  { id: 'paper', emoji: '✋' },
  { id: 'scissors', emoji: '✌️' },
];

function useGameMove(game: Pick<GameResponse, 'id' | 'message_id'>) {
  const applyGameUpdate = useChatStore((s) => s.applyGameUpdate);
  const { t } = useTranslation();
  const [busy, setBusy] = useState(false);

  const move = useCallback(async (payload: GameMoveRequest) => {
    if (busy) return;
    setBusy(true);
    try {
      ...
      toast.danger((err instanceof Error && err.message) || t('game.moveError'));
    } finally {
      setBusy(false);
    }
...
function GameCard({ variant, className = '', children }: { variant: GameVariant; className?: string; children: ReactNode }) {
...
interface ResultBannerProps {
  game: Pick<GameResponse, 'status' | 'winner_id' | 'result'>;
  currentUserId?: string | null;
  variant: GameVariant;
}

function ResultBanner({ game, currentUserId, variant }: ResultBannerProps) {
  const { t } = useTranslation();
...
/* ── Tic-Tac-Toe ── */
function TicTacToeBoard({ game, currentUserId, variant }: GameBoardProps<TicTacToeGameResponse>) {
  const { t } = useTranslation();
...
/* ── Rock-Paper-Scissors ── */
function RockPaperScissors({ game, currentUserId, variant }: GameBoardProps<RpsGameResponse>) {
  const { t } = useTranslation();
  const { move, busy } = useGameMove(game);
  const myRole = game.your_role;
  const myChoice = myRole ? game.choices[myRole] : null;
  const opponentRole = myRole === 'player1' ? 'player2' : 'player1';
  const opponentChoice = game.choices[opponentRole];

  const emojiFor = (id: string | null | undefined) => RPS_CHOICES.find((c) => c.id === id)?.emoji;
...
/* ── Hangman ── */
function Hangman({ game, currentUserId, variant }: GameBoardProps<HangmanGameResponse>) {
  const { t } = useTranslation();
  const { move, busy } = useGameMove(game);
  const isGuesser = game.your_role === 'player2';
  const canGuess = isGuesser && game.status === 'active';
  const displayWord = (game.status === 'finished' ? game.word : game.masked_word) ?? game.masked_word;
...
export default function GameMessage({ message, currentUserId, variant = 'other' }: GameMessageProps) {
