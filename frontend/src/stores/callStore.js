...
export const useCallStore = create((set, get) => ({
  ...
  status: 'idle',
  call: null,               // { id, type, conversationId, isGroup, conversationName }
  incoming: null,           // { callId, type, from, conversationId, conversationName }
  ...
  camOff: false,
  sharingScreen: false,
  startedAt: null,          // timestamp ms cuando la llamada pasa a activa
  elapsed: 0,               // segundos transcurridos (para el cronómetro)
  ...
  attach: (userId) => {
    ...
    sock.on('call:incoming', (payload) => get()._onIncoming(payload));
    sock.on('call:peers', ({ userIds }) => get()._onPeers(userIds));
    sock.on('call:peer-joined', ({ userId: uid }) => get()._onPeerJoined(uid));
    sock.on('call:peer-left', ({ userId: uid }) => get()._onPeerLeft(uid));
    sock.on('call:signal', ({ from, data }) => get()._onSignal(from, data));
    sock.on('call:rejected', ({ userId: uid, reason }) => get()._onRejected(uid, reason));
    sock.on('call:cancelled', () => get()._onCancelled());
    sock.on('call:media', ({ userId: uid, kind, enabled }) => get()._onRemoteMedia(uid, kind, enabled));
  },
  ...
  _loadDirectory: async (conversationId) => {
    ...
    try {
      ...
      const dir = {};
      for (const m of data || []) {
        dir[m.user_id] = { display_name: m.display_name, avatar_url: m.avatar_url || null };
      }
  ...
  _upsertParticipant: (uid, patch) => {
    set((s) => ({
      participants: {
        ...s.participants,
        [uid]: { id: uid, ...(s.participants[uid] || {}), ...patch },
      },
    }));
  },
