...
/** Forma parcial de `last_message_metadata` (columna `unknown` en el tipo de
 *  contrato) para los previews de llamada/broadcast que arma esta pantalla. */
...
const CONTENT_TRANSITION = { duration: 0.28, ease: [0.22, 1, 0.36, 1] as const };
...
function contentKeyFor(pathname: string) {
...
function ConversationItem({
  ...
  const metadata = conversation.last_message_metadata as LastMessageMetadata | null | undefined;
  // Call events have no text body — show a localized preview with a lucide icon.
  const callMeta =
    conversation.last_message_type === 'system' && metadata?.event === 'call'
      ? metadata
      : null;
  const callLabel = callMeta
    ...
    : null;
  const CallIcon: LucideIcon = callMeta?.outcome === 'missed' || callMeta?.outcome === 'declined'
    ? PhoneMissed
    : callMeta?.call_type === 'video'
      ? Video
      : Phone;
  ...
  const stickerKind =
    conversation.last_message_type === 'sticker'
      ? metadata?.sticker?.kind || 'sticker'
      : null;
  const isBroadcastPreview = Boolean(
    metadata?.broadcast_msg_id
    || metadata?.broadcast_list_id,
  );
  const broadcastListName = metadata?.broadcast_list_name || null;
    ...
    <div className="flex items-center gap-2 bg-ink-850 px-2 py-2">
      ...
      <div className="flex items-center">
        <Tooltip delay={0}>
          ...
          <Tooltip.Content placement="top"><p>{t('sidebar.settings')}</p></Tooltip.Content>
        </Tooltip>
      </div>
    </div>
  );
    ...
    <div className="echo-sidebar-bg flex h-full w-full flex-col">
      ...
      <div className="flex h-12 shrink-0 items-center justify-between border-b border-white/5 px-4 shadow-sm">
        ...
        <Tooltip delay={0}>
          ...
          <Tooltip.Content placement="bottom"><p>{t('sidebar.newChat')}</p></Tooltip.Content>
        </Tooltip>
      </div>
