...
export const EASE_OUT: [number, number, number, number] = [0.34, 1, 0.64, 1];
export const SPRING_SNAPPY: Transition = { type: 'spring', stiffness: 420, damping: 30 };
export const SPRING_SOFT: Transition = { type: 'spring', stiffness: 320, damping: 28 };
export const SPRING_BOUNCY: Transition = { type: 'spring', stiffness: 380, damping: 22 };

export function msgEntryTransition(isOwn: boolean, reducedMotion: boolean | null): Transition {
  if (reducedMotion) return { duration: 0.01 };
  return isOwn ? SPRING_SNAPPY : { duration: 0.24, ease: EASE_OUT };
}

export function msgEntryInitial(
  isOwn: boolean,
...
export function listItemEntry(index: number, reducedMotion: boolean | null): ListItemEntry {
