 ...
 * @param {'gif'|'sticker'} kind
 */
export async function giphyTrending(kind, { limit = 24, offset = 0, signal } = {}) {
 ...
 * @param {'gif'|'sticker'} kind
 */
export async function giphySearch(kind, query, { limit = 24, offset = 0, signal } = {}) {
  const q = (query || '').trim();
