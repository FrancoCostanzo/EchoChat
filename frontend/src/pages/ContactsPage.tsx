...
import type { RelationshipEntry } from '@/types/relationship';
...
type ContactUser = RelationshipEntry & { is_favorite?: boolean };

/** Relationship rows use `target_user_id`; search results use `id`. */
function targetIdOf(user: ContactUser | UserResponse): string {
  return 'target_user_id' in user ? user.target_user_id : user.id;
}

interface ContactCardProps {
  user: ContactUser;
  type: 'contact' | 'blocked';
  onRemove: (user: ContactUser) => void;
  onToggleFavorite: (user: ContactUser) => void;
  onMessage: (user: ContactUser) => void;
  messaging: boolean;
}

function ContactCard({ user, type, onRemove, onToggleFavorite, onMessage, messaging }: ContactCardProps) {
        ...
        {type === 'contact' && (
          <>
            <Tooltip delay={200}>
              ...
              <Tooltip.Content placement="top"><p>{t('contacts.sendMessage')}</p></Tooltip.Content>
            </Tooltip>
            <Tooltip delay={200}>
              <Button
                isIconOnly
...
function AddUserRow({ user, isContact, adding, onAdd, t }: AddUserRowProps) {
  return (
...
export default function ContactsPage() {
  ...
  const [contacts, setContacts] = useState<RelationshipEntry[]>([]);
  const [favorites, setFavorites] = useState<RelationshipEntry[]>([]);
  ...
  const loadPage = useCallback(async (currentOffset: number, replace = false) => {
    setPageLoading(true);
    try {
  ...
  const addContact = async (user: UserResponse) => {
  ...
  const removeContact = async (user: ContactUser) => {
    const name = user.display_name || user.username;
    try {
      ...
      toast.danger((err instanceof Error && err.message) || t('contacts.removeError'), {
        indicator: <AlertCircle size={16} />,
      });
    }
  };

  const toggleFavorite = async (user: ContactUser) => {
    ...
    try {
      ...
      toast.danger((err instanceof Error && err.message) || t('contacts.favoriteError'), {
        indicator: <AlertCircle size={16} />,
      });
    }
  };

  const unblock = async (user: ContactUser) => {
    const name = user.display_name || user.username;
    try {
      ...
      toast.danger((err instanceof Error && err.message) || t('contacts.unblockError'), {
        indicator: <AlertCircle size={16} />,
      });
    }
  };

  const messageContact = async (user: ContactUser) => {
    const id = targetIdOf(user);
    setMessagingId(id);
    try {
      ...
      toast.danger((err instanceof Error && err.message) || t('contacts.messageError'), {
        indicator: <AlertCircle size={16} />,
      });
    } finally {
      setMessagingId(null);
