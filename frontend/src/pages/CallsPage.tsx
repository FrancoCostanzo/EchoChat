...
function CallsEmptyState({ tab, t }: { tab: string; t: TFunc }) {
...
interface CallRowProps {
  ...
  onOpen: (call: CallHistoryItem) => void;
  onCallAgain: (call: CallHistoryItem) => void;
  t: TFunc;
}

function CallRow({ call, selfId, onOpen, onCallAgain, t }: CallRowProps) {
  ...
  return (
    <div className="group flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-ink-750">
      ...
      >
        {isDirect ? (
          // CallHistoryItem trae avatar_key (object key sin resolver), no una
          // URL — igual que antes, este avatar cae siempre a las iniciales.
          <UserAvatar user={{ display_name: name }} size="sm" />
        ) : (
      ...
      <Tooltip delay={200}>
        ...
        <Tooltip.Content placement="top"><p>{t('call.history.callAgain')}</p></Tooltip.Content>
      </Tooltip>
    </div>
  );
}
...
export default function CallsPage() {
  ...
  const openConversation = useCallback((call: CallHistoryItem) => {
    if (call.conversation_id) navigate(`/chat/${call.conversation_id}`);
  }, [navigate]);

  const callAgain = useCallback(async (call: CallHistoryItem) => {
