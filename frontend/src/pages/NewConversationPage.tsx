...
const JOIN_MODES: { id: JoinMode; Icon: typeof Globe; labelKey: string }[] = [
...
const CATEGORIES: ChannelCategory[] = ['general', 'announcements', 'department', 'project'];

/* ── helpers ─────────────────────────────────────────────────── */
function SelectedChip({ user, onRemove }: { user: UserResponse; onRemove: (id: string) => void }) {
...
function UserRow({
  ...
  user: UserResponse;
  isSelected: boolean;
  onToggle: (user: UserResponse) => void;
  isMulti: boolean;
}) {
  return (
    <motion.button
